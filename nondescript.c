...
struct TypeDescriptor{
    ...
    bool (*hash)(NDSContext *ctx, NDSValue v, uint32_t *out);
...
#define NDS_NO_GLOBAL UINT32_MAX
#define NDS_NO_CONSTANT UINT32_MAX
#define NDS_NO_COMMAND UINT32_MAX
#define NDS_NO_FUNCTION UINT32_MAX
...
struct NDSContext{
    ...
    /* Import state */
    uint32_t importedNames[MAX_IMPORTED_FILES]; /* constant-pool indices */
...
static NDSStatus
Value_equals(NDSContext *ctx, NDSValue a, NDSValue b, bool *out);
static void
...
static inline NDSValue
NDSValue_functionFromIndex(uint32_t idx)
{
    NDSValue v = {0};
...
static char *
NDSContext_newStringUninit(NDSContext *self, size_t length, NDSValue *dest)
{
    /* Direct gcAllocate — strings are created before type registration which means the string
...
{
    char *buf = NDSContext_newStringUninit(self, length, dest);
    if (!buf)
        return false;
...
Atoms_reserveSlot(NDSContext *self)
{
    ...
    if (self->atomCount >= self->atomCapacity){
        ...
        size_t mapCap = oldCap;
        if (!Util_growArray(self->allocator, (void **)&self->atomToGlobal, &mapCap, sizeof(uint32_t)))
            return NULL;
        for (size_t i = oldCap; i < mapCap; i++)
            self->atomToGlobal[i] = NDS_NO_GLOBAL;
...
NDSContext_addStringConstant(NDSContext *self, const char *bytes, size_t length)
{
    ...
    if (!slot)
        return NDS_NO_CONSTANT;
    if (!NDSContext_newString(self, bytes, length, slot)){
        self->constantCount--;
        return NDS_NO_CONSTANT;
    }
    return (uint32_t)(self->constantCount - 1);
}
...
NDSContext_addConstant(NDSContext *self, NDSValue value)
{
    ...
    uint32_t index = (uint32_t)self->constantCount++;
...
{
    if (name >= ctx->atomCount)
        return NDS_NO_GLOBAL;
    return ctx->atomToGlobal[name];
}

static uint32_t
Globals_ensure(NDSContext *ctx, NDSAtom name)
{
    uint32_t idx = Globals_find(ctx, name);
    if (idx != NDS_NO_GLOBAL)
        return idx;
...
{
    uint32_t idx = Globals_ensure(ctx, name);
    if (idx != NDS_NO_GLOBAL)
...
static NDSStatus
Type_invalidChunkID(NDSContext *ctx, const char *typeName, NDSChunkID chunkID)
{
    NDSContext_setErrorF(ctx, "'%s' is not a valid chunk type for %s",
                         NDSContext_atomName(ctx, chunkID), typeName);
...
static bool
Hash_value(NDSContext *ctx, NDSValue v, uint32_t *out)
{
    TypeDescriptor *desc = NDSContext_getTypeDesc(ctx, v.type);
    return desc->hash && desc->hash(ctx, v, out);
}
...
static bool
Boolean_hash(NDSContext *ctx, NDSValue v, uint32_t *out)
{
    UNUSED(ctx);
    *out = (uint32_t)(v.as.boolean);
    return true;
}
...
static bool
Number_hash(NDSContext *ctx, NDSValue v, uint32_t *out)
{
    UNUSED(ctx);
    double d = v.as.number;
    int fpclass = fpclassify(d);
    if (fpclass != FP_NORMAL && fpclass != FP_ZERO && fpclass != FP_SUBNORMAL)
        return false;
    if (d == 0.0)
        d = 0.0;
    *out = Hash_fnv1a(&d, sizeof(d));
    return true;
}
...
static bool
String_hash(NDSContext *ctx, NDSValue v, uint32_t *out)
{
    if (!v.as.object){
        *out = 0;
        return true;
    ...
    *out = Hash_fnv1a(s->data, s->length);
    return true;
}
...
String_chunkGet(NDSContext *ctx, NDSChunkID chunkID)
{
    ...
    return Type_invalidChunkID(ctx, "string", chunkID);
}
...
String_chunkGetRange(NDSContext *ctx, NDSChunkID chunkID)
{
    ...
    if (chunkID == NDSBuiltinAtom_Character){
        ...
        if (endIdx < startIdx){
            NDSContext_setError(ctx, "range end is before range start");
            return NDSStatus_Error;
        }
...
List_equals(NDSContext *ctx)
{
    ...
    if (a->count != b->count)
        ...
        for (size_t i = 0; i < a->count; i++){
            bool itemEq = false;
            if (Value_equals(ctx, a->items[i], b->items[i], &itemEq) != NDSStatus_OK)
                return NDSStatus_Error;
            if (!itemEq){
                eq = false;
                break;
...
List_contains(NDSContext *ctx)
{
    ...
    for (size_t i = 0; i < list->count; i++){
        bool itemEq = false;
        if (Value_equals(ctx, list->items[i], needle, &itemEq) != NDSStatus_OK)
            return NDSStatus_Error;
        if (itemEq){
            found = true;
            break;
...
static NDSStatus
List_resolveItemIndex(NDSContext *ctx, NDSChunkID chunkID, int idxSlot,
                      List **outList, size_t *outIdx)
{
    if (chunkID != NDSBuiltinAtom_Item)
        return Type_invalidChunkID(ctx, "list", chunkID);
    if (NDSContext_getSlotType(ctx, idxSlot) != NDSValueType_Number){
        NDSContext_setError(ctx, "expected a number");
        return NDSStatus_Error;
    }
    *outList = NDSContext_getSlotList(ctx, 0);
    if (!Util_resolveIndex(NDSContext_getSlotNumber(ctx, idxSlot), (*outList)->count, outIdx)){
        NDSContext_setError(ctx, "index out of range");
        return NDSStatus_Error;
    }
    return NDSStatus_OK;
...
static NDSStatus
List_resolveItemRange(NDSContext *ctx, NDSChunkID chunkID,
                      List **outList, size_t *outStart, size_t *outEnd)
{
    if (chunkID != NDSBuiltinAtom_Item)
        return Type_invalidChunkID(ctx, "list", chunkID);
    if (NDSContext_getSlotType(ctx, 1) != NDSValueType_Number ||
    ...
    *outList = NDSContext_getSlotList(ctx, 0);
    if (!Util_resolveIndex(NDSContext_getSlotNumber(ctx, 1), (*outList)->count, outStart) ||
        !Util_resolveIndex(NDSContext_getSlotNumber(ctx, 2), (*outList)->count, outEnd)){
        NDSContext_setError(ctx, "index out of range");
        return NDSStatus_Error;
    }
    if (*outEnd < *outStart){
...
List_chunkGet(NDSContext *ctx, NDSChunkID chunkID)
{
    ...
    List *list = NULL;
    size_t idx = 0;
    if (List_resolveItemIndex(ctx, chunkID, 1, &list, &idx) != NDSStatus_OK)
        return NDSStatus_Error;
...
List_chunkGetRange(NDSContext *ctx, NDSChunkID chunkID)
{
    ...
    if (List_resolveItemRange(ctx, chunkID, &list, &startIdx, &endIdx) != NDSStatus_OK)
        return NDSStatus_Error;
...
/*
 * indices: List<number-or-nothing> — nothing = empty bucket; probes terminate here.
 * entries: List<MapEntry-or-nothing> — nothing = tombstone; probes walk past it.
 */

#define MAP_NO_ENTRY SIZE_MAX
...
static List *
Map_entries(const NDSContext *ctx, const Map *map)
{
    return (List *)NDSObject_getExtraData(ctx, map->entries);
}

static List *
Map_indices(const NDSContext *ctx, const Map *map)
{
    return (List *)NDSObject_getExtraData(ctx, map->indices);
}

static bool
Map_indexAt(const NDSContext *ctx, const Map *map, size_t bucket, size_t *outEntryIdx)
{
    NDSValue v = Map_indices(ctx, map)->items[bucket];
    if (v.type == NDSValueType_Nothing)
        return false;
    *outEntryIdx = (size_t)v.as.number;
    return true;
}

static bool
Map_entryIsLive(const NDSContext *ctx, const Map *map, size_t entryIdx)
{
    return Map_entries(ctx, map)->items[entryIdx].type != NDSValueType_Nothing;
}

static NDSValue
Map_entryKey(const NDSContext *ctx, const Map *map, size_t entryIdx)
{
    NDSObject *e = Map_entries(ctx, map)->items[entryIdx].as.object;
    return NDSObject_getProperty(e, MapEntryProp_Key);
}

static NDSValue
Map_entryValue(const NDSContext *ctx, const Map *map, size_t entryIdx)
{
    NDSObject *e = Map_entries(ctx, map)->items[entryIdx].as.object;
    return NDSObject_getProperty(e, MapEntryProp_Value);
}

static NDSStatus
Map_probe(NDSContext *ctx, const Map *map, NDSValue key, uint32_t hash,
          size_t *outBucket, size_t *outEntryIdx)
{
    size_t mask = Map_indices(ctx, map)->count - 1;
    size_t bucket = hash & mask;
    while (true){
        size_t entryIdx;
        if (!Map_indexAt(ctx, map, bucket, &entryIdx)){
            *outBucket = bucket;
            *outEntryIdx = MAP_NO_ENTRY;
            return NDSStatus_OK;
        }
        if (Map_entryIsLive(ctx, map, entryIdx)){
            bool keyEq;
            if (Value_equals(ctx, Map_entryKey(ctx, map, entryIdx), key, &keyEq) != NDSStatus_OK)
                return NDSStatus_Error;
            if (keyEq){
                *outBucket = bucket;
...
static void
Map_rehash(NDSContext *ctx, Map *map)
{
    List *indices = Map_indices(ctx, map);
    List *entries = Map_entries(ctx, map);
    size_t mask = indices->count - 1;
    for (size_t i = 0; i < indices->count; i++)
        indices->items[i] = NDSValue_Nothing;
    for (size_t i = 0; i < entries->count; i++){
        if (entries->items[i].type == NDSValueType_Nothing)
            continue;
        NDSValue key = NDSObject_getProperty(entries->items[i].as.object, MapEntryProp_Key);
        uint32_t hash;
        Hash_value(ctx, key, &hash); /* keys in the entries list were validated at insert */
        size_t bucket = hash & mask;
        while (indices->items[bucket].type != NDSValueType_Nothing)
            bucket = (bucket + 1) & mask;
        indices->items[bucket] = NDSValue_numberFromDouble((double)i);
    }
}

static void
Map_compact(NDSContext *ctx, Map *map)
{
    List *entries = Map_entries(ctx, map);
    size_t dst = 0;
    for (size_t src = 0; src < entries->count; src++){
        if (entries->items[src].type == NDSValueType_Nothing)
            continue;
        if (dst != src)
            entries->items[dst] = entries->items[src];
        dst++;
    }
    entries->count = dst;
    Map_rehash(ctx, map);
...
static bool
Map_growIndices(NDSContext *ctx, NDSObject *obj)
{
    Map *map = (Map *)NDSObject_getExtraData(ctx, obj);

    if (!map->indices){
        int scratch = NDSContext_getSlotCount(ctx);
        if (NDSContext_ensureSlots(ctx, scratch + 1) != NDSStatus_OK)
            return false;
        NDSValue *tmpSlot = NDSContext_slotPointer(ctx, scratch);

        if (!List_new(ctx, tmpSlot)){
            ctx->stackTop--;
            return false;
        }
        map = (Map *)NDSObject_getExtraData(ctx, obj);
        map->indices = tmpSlot->as.object;
        ...
        map = (Map *)NDSObject_getExtraData(ctx, obj);
        map->entries = tmpSlot->as.object;
        ctx->stackTop--;

        for (size_t i = 0; i < MAP_INITIAL_BUCKET_COUNT; i++)
            if (!List_append(ctx, map->indices, NDSValue_Nothing))
                return false;
        return true;
    }

    List *entries = Map_entries(ctx, map);
    if (entries->count > map->count)
        Map_compact(ctx, map);

    List *indices = Map_indices(ctx, map);
    if ((entries->count + 1) * 100 <= indices->count * MAP_MAX_LOAD_PERCENT)
        return true;

    size_t newCount;
    if (!Util_safeMulSize(indices->count, 2, &newCount))
        return false;
    while (Map_indices(ctx, map)->count < newCount)
        if (!List_append(ctx, map->indices, NDSValue_Nothing))
            return false;
    Map_rehash(ctx, map);
    return true;
...
Map_set(NDSContext *ctx, NDSObject *obj, NDSValue key, NDSValue value)
{
    ...
    uint32_t hash;
    if (!Hash_value(ctx, key, &hash))
        return -1;

    if (!map->indices && !Map_growIndices(ctx, obj))
        return -1;

    map = (Map *)NDSObject_getExtraData(ctx, obj);
    size_t entryIdx;
    size_t bucket;
    if (Map_probe(ctx, map, key, hash, &bucket, &entryIdx) != NDSStatus_OK)
        return -1;

    if (entryIdx != MAP_NO_ENTRY){
        NDSObject *e = Map_entries(ctx, map)->items[entryIdx].as.object;
        NDSObject_setProperty(e, MapEntryProp_Value, value);
        return 0;
    }

    /* Root the new MapEntry in a scratch slot until List_append takes ownership. */
    int scratch = NDSContext_getSlotCount(ctx);
    if (NDSContext_ensureSlots(ctx, scratch + 1) != NDSStatus_OK)
        return -1;
    NDSValue *entrySlot = NDSContext_slotPointer(ctx, scratch);
    if (!MapEntry_new(ctx, key, value, entrySlot)){
        ctx->stackTop--;
        return -1;
    }
    map = (Map *)NDSObject_getExtraData(ctx, obj);

    size_t newEntryIdx = Map_entries(ctx, map)->count;
    if (!List_append(ctx, map->entries, *entrySlot)){
        ctx->stackTop--;
        return -1;
    }
    ctx->stackTop--;
    map = (Map *)NDSObject_getExtraData(ctx, obj);
    Map_indices(ctx, map)->items[bucket] = NDSValue_numberFromDouble((double)newEntryIdx);
    map->count++;

    if ((Map_entries(ctx, map)->count + 1) * 100 > Map_indices(ctx, map)->count * MAP_MAX_LOAD_PERCENT)
        if (!Map_growIndices(ctx, obj))
            return -1;
    return 1;
}

static NDSStatus
Map_findEntry(NDSContext *ctx, const Map *map, NDSValue key, size_t *outEntryIdx)
{
    uint32_t hash;
    if (!map->indices || map->count == 0 || !Hash_value(ctx, key, &hash)){
        *outEntryIdx = MAP_NO_ENTRY;
        return NDSStatus_OK;
    }
    size_t bucket;
    return Map_probe(ctx, map, key, hash, &bucket, outEntryIdx);
}

static NDSStatus
Map_get(NDSContext *ctx, NDSObject *obj, NDSValue key, NDSValue *outValue, bool *outFound)
{
    Map *map = (Map *)NDSObject_getExtraData(ctx, obj);
    size_t entryIdx;
    if (Map_findEntry(ctx, map, key, &entryIdx) != NDSStatus_OK){
        *outFound = false;
        return NDSStatus_Error;
    }
    if (entryIdx == MAP_NO_ENTRY){
    ...
    if (outValue)
        *outValue = Map_entryValue(ctx, map, entryIdx);
    *outFound = true;
    return NDSStatus_OK;
}

static NDSStatus
Map_remove(NDSContext *ctx, NDSObject *obj, NDSValue key, bool *outRemoved)
{
    Map *map = (Map *)NDSObject_getExtraData(ctx, obj);
    size_t entryIdx;
    if (Map_findEntry(ctx, map, key, &entryIdx) != NDSStatus_OK){
    ...
    Map_entries(ctx, map)->items[entryIdx] = NDSValue_Nothing;
    map->count--;

    if (Map_entries(ctx, map)->count > 2 * map->count)
        Map_compact(ctx, map);
    *outRemoved = true;
    return NDSStatus_OK;
}
...
Map_trace(NDSContext *ctx, NDSObject *obj)
{
    Map *map = (Map *)NDSObject_getExtraData(ctx, obj);
    if (map->indices)
        NDSContext_gcPushGray(ctx, map->indices);
    if (map->entries)
        NDSContext_gcPushGray(ctx, map->entries);
}
...
Map_equals(NDSContext *ctx)
{
    ...
    if (a->count != b->count)
        ...
        List *entries = Map_entries(ctx, a);
        for (size_t i = 0; i < entries->count; i++){
            if (entries->items[i].type == NDSValueType_Nothing)
                continue;
            NDSValue val = {0};
            NDSValue key = NDSObject_getProperty(entries->items[i].as.object, MapEntryProp_Key);
            NDSValue aVal = NDSObject_getProperty(entries->items[i].as.object, MapEntryProp_Value);
            bool found = false;
            if (Map_get(ctx, bObj, key, &val, &found) != NDSStatus_OK)
                return NDSStatus_Error;
            if (!found){
                eq = false;
                break;
            ...
            bool valEq;
            if (Value_equals(ctx, aVal, val, &valEq) != NDSStatus_OK)
                return NDSStatus_Error;
            if (!valEq){
                eq = false;
                break;
...
Map_contains(NDSContext *ctx)
{
    ...
    bool found = false;
    if (Map_get(ctx, containerVal.as.object, key, NULL, &found) != NDSStatus_OK)
        return NDSStatus_Error;
    NDSContext_setSlotBoolean(ctx, 0, found);
    return NDSStatus_OK;
}
...
Map_toString(NDSContext *ctx)
{
    ...
    if (map->entries){
        ...
        for (size_t i = 0; i < entries->count; i++){
            ...
            NDSObject *e = entries->items[i].as.object;
            *NDSContext_slotPointer(ctx, 1) = NDSObject_getProperty(e, MapEntryProp_Key);
            if (NDSContext_slotToString(ctx, 1, 1) != NDSStatus_OK)
                break;
            StringData *s = NDSContext_getSlotStringData(ctx, 1);
            ...
            *NDSContext_slotPointer(ctx, 2) = NDSObject_getProperty(e, MapEntryProp_Value);
...
Map_chunkGet(NDSContext *ctx, NDSChunkID chunkID)
{
    if (chunkID != NDSBuiltinAtom_Item && chunkID != NDSBuiltinAtom_Key)
        return Type_invalidChunkID(ctx, "map", chunkID);
    ...
    NDSValue result;
    bool found = false;
    if (Map_get(ctx, containerVal.as.object, key, &result, &found) != NDSStatus_OK)
        return NDSStatus_Error;
    if (!found){
...
Map_chunkGetEvery(NDSContext *ctx, NDSChunkID chunkID)
{
    if (chunkID != NDSBuiltinAtom_Item && chunkID != NDSBuiltinAtom_Value &&
        chunkID != NDSBuiltinAtom_Key && chunkID != NDSBuiltinAtom_Entry)
        return Type_invalidChunkID(ctx, "map", chunkID);
    ...
    if (map->entries){
        ...
        for (size_t i = 0; i < entries->count; i++){
            ...
            NDSObject *e = entries->items[i].as.object;
            NDSValue v;
            if (chunkID == NDSBuiltinAtom_Entry)
                v = entries->items[i];
            else if (chunkID == NDSBuiltinAtom_Key)
                v = NDSObject_getProperty(e, MapEntryProp_Key);
            else
                v = NDSObject_getProperty(e, MapEntryProp_Value);
            if (!List_append(ctx, resultObj, v)){
                NDSContext_setError(ctx, "out of memory");
                return NDSStatus_Error;
            }
        }
...
Map_chunkDelete(NDSContext *ctx, NDSChunkID chunkID)
{
    ...
    bool removed;
    if (Map_remove(ctx, containerVal.as.object, key, &removed) != NDSStatus_OK)
        return NDSStatus_Error;
    if (!removed){
        NDSContext_setError(ctx, "key not found");
        return NDSStatus_Error;
...
Error_equals(NDSContext *ctx)
{
    ...
    bool typeEq, msgEq, valEq;
    if (Value_equals(ctx, NDSObject_getProperty(a.as.object, ErrorProp_Type),
                     NDSObject_getProperty(b.as.object, ErrorProp_Type), &typeEq) != NDSStatus_OK)
        return NDSStatus_Error;
    if (Value_equals(ctx, NDSObject_getProperty(a.as.object, ErrorProp_Message),
                     NDSObject_getProperty(b.as.object, ErrorProp_Message), &msgEq) != NDSStatus_OK)
        return NDSStatus_Error;
    if (Value_equals(ctx, NDSObject_getProperty(a.as.object, ErrorProp_Value),
                     NDSObject_getProperty(b.as.object, ErrorProp_Value), &valEq) != NDSStatus_OK)
        return NDSStatus_Error;

    NDSContext_setSlotBoolean(ctx, 0, typeEq && msgEq && valEq);
    return NDSStatus_OK;
}
...
static NDSStatus
Type_dispatchPropertyGet(NDSContext *ctx, NDSAtom nameAtom)
{
    NDSValue containerVal = NDSContext_getSlot(ctx, 0);
    ...
    if (desc->property_get)
        return desc->property_get(ctx, nameAtom);
...
registerTypeInternal(NDSContext *ctx, const NDSTypeDescriptor *pub)
{
    ...
    td->property_get = pub->property_get; /* NULL = no user hook beyond defaults */
    ...
    td->hash = NULL; /* host-registered types not hashable yet; needs NDSValue in the public API */
...
Map_gcSize(NDSContext *ctx, NDSObject *obj)
{
    (void)ctx;
    (void)obj;
    return sizeof(NDSObject) + sizeof(Map);
}
...
NDSContext_registerStandardTypes(NDSContext *ctx)
{
    ...
    ctx->types[NDSValueType_Boolean].hash = Boolean_hash;
    ctx->types[NDSValueType_Number].hash = Number_hash;
    ctx->types[NDSValueType_String].hash = String_hash;
...
typedef struct{
    ...
    /* for expressions, stop if the precedence is <= this;
     * lets us use keyword ops in a command after an expression without them getting
     * absorbed, which is nice
     */
    Precedence parsePrec;
} InternalPatternStep;

#define NDS_MAX_HOST_COMMAND_VARS 64
...
struct HostCommand{
    ...
    int expressionCount;
    int variableCount;
    uint64_t referenceMask; /* bit set if a variable is also an in/out reference */
    uint32_t functionIndex;
};

struct CommandTrieNode{
    NDSAtom atom;
    uint32_t commandIndex; /* NDS_NO_COMMAND = interior node, no command terminates here */
    CommandTrieNode *children;
    CommandTrieNode *next;
};

static NDSStatus
CommandTrie_insert(NDSContext *ctx, uint32_t commandIndex)
{
    ...
    for (int i = 0; i < cmd->headLength; i++){
        ...
        if (node){
            if (!isLast && node->commandIndex != NDS_NO_COMMAND)
                return NDSStatus_Error;
            if (isLast && node->children)
                return NDSStatus_Error;
            if (isLast){
                if (node->commandIndex != NDS_NO_COMMAND)
                    return NDSStatus_Error;
            ...
            if (!newNode)
                return NDSStatus_Error;
            newNode->atom = atom;
            newNode->commandIndex = isLast? commandIndex : NDS_NO_COMMAND;
            newNode->next = *list;
            *list = newNode;
...
FunctionTable_add(NDSContext *ctx, Function func)
{
    ...
    uint32_t idx = (uint32_t)ctx->functionCount++;
...
NDSContext_new(const NDSConfig *config)
{
    ...
    ALLOC_TABLE(atomToGlobal, atomCapacity, INITIAL_ATOM_CAPACITY, uint32_t);
    for (size_t i = 0; i < ctx->atomCapacity; i++)
        ctx->atomToGlobal[i] = NDS_NO_GLOBAL;
...
#define INIT_CHECK(expr)                                                                           \
    ...
    INIT_CHECK(Globals_ensure(ctx, NDSBuiltinAtom_It) != NDS_NO_GLOBAL);
...
NDSContext_free(NDSContext *self)
{
    ...
    NDSAllocator_free(alloc, self->atomToGlobal);
...
NDSConfigHandle_registerFunction(NDSConfigHandle *h, const char *name, NDSHostFunctionCallback callback, int paramCount)
{
    ...
    uint32_t funcIdx = FunctionTable_add(ctx, func);
    if (funcIdx == NDS_NO_FUNCTION)
        return NDSStatus_Error;
    uint32_t slot = Globals_ensureConst(ctx, atom);
    if (slot == NDS_NO_GLOBAL)
        return NDSStatus_Error;
...
/* Intern a name and ensure a const global slot for it.
   Returns the global index, or NDS_NO_GLOBAL on failure. */
static uint32_t
...
{
    uint32_t slot = ConfigHandle_ensureConstSlot(h, name);
    if (slot == NDS_NO_GLOBAL)
        return NDSStatus_Error;
...
NDSConfigHandle_registerCommand(NDSConfigHandle *h, const NDSPatternStep *pattern, NDSHostFunctionCallback callback)
{
    ...
    /* Count steps and validate, update reference mask. */
    int stepCount = 0;
    int expressionCount = 0;
    int variableCount = 0;
    uint64_t referenceMask = 0;
    for (int i = 0; pattern[i].kind != NDSPatternStep_End; i++){
        if (i >= 256)
            return NDSStatus_Error;
        if (pattern[i].kind == NDSPatternStep_Expression)
            expressionCount++;
        else if (pattern[i].kind == NDSPatternStep_Variable || pattern[i].kind == NDSPatternStep_Reference){
            if (variableCount >= NDS_MAX_HOST_COMMAND_VARS)
                return NDSStatus_Error;
            if (pattern[i].kind == NDSPatternStep_Reference)
                referenceMask |= (uint64_t)1 << variableCount;
            variableCount++;
    ...
    for (int i = 0; i < stepCount; i++){
        ...
        case NDSPatternStep_Variable:
            ...
            inHead = false;
            break;
        case NDSPatternStep_Reference:
            steps[i].kind = PatternStepKind_Reference;
            inHead = false;
            break;
    ...
    NDSAtom andAtom      = NDSContext_internAtom(ctx, "and");
    NDSAtom orAtom       = NDSContext_internAtom(ctx, "or");
    NDSAtom asAtom       = NDSContext_internAtom(ctx, "as");
    NDSAtom isAtom       = NDSContext_internAtom(ctx, "is");
    NDSAtom containsAtom = NDSContext_internAtom(ctx, "contains");
    NDSAtom divAtom      = NDSContext_internAtom(ctx, "div");
    NDSAtom modAtom      = NDSContext_internAtom(ctx, "mod");
    for (int i = 0; i < stepCount - 1; i++){
        if (steps[i].kind != PatternStepKind_Expression || steps[i+1].kind != PatternStepKind_Word)
            continue;
        NDSAtom next = steps[i+1].atom;
        /* FIXME - this is a bit of a mess...definitely fragile in terms of new word ops being added... */
        Precedence ceiling = Precedence_None;
        if      (next == orAtom)
            ceiling = Precedence_And;
        else if (next == andAtom)
            ceiling = Precedence_Not;
        else if (next == isAtom || next == containsAtom)
            ceiling = Precedence_Concat;
        else if (next == divAtom || next == modAtom)
            ceiling = Precedence_Unary;
        else if (next == asAtom)
            ceiling = Precedence_OfExpr;
        steps[i].parsePrec = ceiling;
    }
    ...
    /* Store command */
    uint32_t cmdIndex = (uint32_t)ctx->commandCount++;
    ...
    cmd->referenceMask = referenceMask;
...
#define VM_DISPATCH_SETUP(ctx_, nslots_, on_fail_)                                                 \
    ...
    if ((ctx_)->callDepth >= VM_MAX_CALL_DEPTH){                                                   \
        NDSContext_setError((ctx_), "stack overflow (too many nested calls)");                     \
        on_fail_;                                                                                  \
    }                                                                                              \
...
NDSContext_getSlotMapValue(NDSContext *self, int mapSlot, int keySlot, int destSlot)
{
    ...
    NDSValue result = {0};
    bool found = false;
    if (Map_get(self, mv->as.object, *kv, &result, &found) == NDSStatus_OK && found){
        *dest = result;
        return true;
    }
    *dest = NDSValue_Nothing;
...
NDSContext_removeSlotMapValue(NDSContext *self, int mapSlot, int keySlot)
{
    ...
    if (mv->type != NDSValueType_Map || !mv->as.object)
        return false;
    bool removed;
    if (Map_remove(self, mv->as.object, *kv, &removed) != NDSStatus_OK)
        return false;
    return removed;
}
...
NDSContext_getSlotMapContainsKey(NDSContext *self, int mapSlot, int keySlot)
{
    ...
    if (mv->type != NDSValueType_Map || !mv->as.object)
        return false;
    bool found = false;
    if (Map_get(self, mv->as.object, *kv, NULL, &found) != NDSStatus_OK)
        return false;
    return found;
}
...
NDSContext_getNextSlotMapEntry(NDSContext *self, int mapSlot, NDSMapIterCookie *cookie, int keySlot, int valueSlot)
{
    ...
    List *entries = Map_entries(self, map);
    for (size_t i = *cookie; i < entries->count; i++){
        if (entries->items[i].type != NDSValueType_Nothing){
            NDSObject *e = entries->items[i].as.object;
            *kd = NDSObject_getProperty(e, MapEntryProp_Key);
            *vd = NDSObject_getProperty(e, MapEntryProp_Value);
            *cookie = i + 1;
            return true;
        }
    }
...
typedef enum{
    ...
    Op_FillSlot,      /* .a = depth; pop top, write to stack[stackTop - 1 - depth] */
...
static void NDS_PRINTF(2, 3)
Compiler_error(Compiler *self, const char *fmt, ...)
{
    ...
    if (self->tokenCount > 0){
        ...
        ctx->errorAtEOF = self->tokens[idx].type == Token_EOF;
    }
...
{
    uint32_t index = NDSContext_addConstant(self->ctx, value);
    if (index == NDS_NO_CONSTANT)
        Compiler_error(self, "too many constants");
    Compiler_emit(self, Op_PushConst, index, 0);
}

static void
Compiler_emitStringConstant(Compiler *self, const char *bytes, size_t length)
{
    uint32_t index = NDSContext_addStringConstant(self->ctx, bytes, length);
    if (index == NDS_NO_CONSTANT)
        Compiler_error(self, "out of memory");
    Compiler_emit(self, Op_PushConst, index, 0);
}
...
Compiler_resolveVar(Compiler *self, NDSAtom atom)
{
    ...
    uint32_t g = Globals_find(self->ctx, atom);
    if (g != NDS_NO_GLOBAL)
...
Compiler_emitStoreVarByAtom(Compiler *self, NDSAtom atom)
{
    ...
    if (self->scopeDepth > 0){
        ...
        uint32_t globalSlot = Globals_ensure(self->ctx, atom);
        if (globalSlot == NDS_NO_GLOBAL)
            Compiler_error(self, "out of memory");
        Compiler_emit(self, Op_StoreGlobal, globalSlot, 0);
    }
}
...
Compiler_parseGivenBlock(Compiler *self, size_t callIdx)
{
    ...
    uint32_t funcIndex = FunctionTable_add(self->ctx, func);
    ...
    if (funcIndex == NDS_NO_FUNCTION)
        Compiler_error(self, "out of memory");

    /* Patch the Op_Call instruction's .b to attach the block */
    self->code.code[callIdx].b = funcIndex;
    PARSE_LEAVE;
}
...
{
    const char *varNames[NDS_MAX_HOST_COMMAND_VARS] = {0};
    size_t varNameLens[NDS_MAX_HOST_COMMAND_VARS] = {0};
    int varIdx = 0;
    ...
    for (int i = cmd->headLength; i < cmd->stepCount; i++){
        ...
        case PatternStepKind_Expression:
            if (step->parsePrec != Precedence_None)
                Compiler_parsePrecedence(self, step->parsePrec);
            else
                Compiler_parseExpression(self);
            break;
        case PatternStepKind_Variable:
        case PatternStepKind_Reference:{
            Token *tok = Compiler_current(self);
            if (tok->type != Token_Word)
    ...
    for (int i = 0; i < cmd->variableCount; i++){
        if (cmd->referenceMask & ((uint64_t)1 << i)){
            Compiler_emitLoadVar(self, varNames[i], varNameLens[i]);
            Compiler_emit(self, Op_FillSlot, (uint32_t)(cmd->expressionCount + i), 0);
        }
    }
...
Compiler_parseHostCommand(Compiler *self)
{
    ...
    while (node){
        ...
        if (child->commandIndex != NDS_NO_COMMAND){
...
Compiler_parseFunctionDefinition(Compiler *self)
{
    ...
    /* Reserve the global slot so the name is resolvable during body
       ...
       function value is stored at runtime via emitted bytecode below. */
    uint32_t globalSlot = Globals_ensure(self->ctx, nameAtom);
    if (globalSlot == NDS_NO_GLOBAL)
        Compiler_error(self, "out of memory");
    ...
    /* Store the function into the global. */
    Compiler_emitConstant(self, NDSValue_functionFromIndex(funcIndex));
    Compiler_emit(self, Op_StoreGlobal, globalSlot, 0);
    PARSE_LEAVE;
}
...
Compiler_parseImport(Compiler *self)
{
    ...
    uint32_t nameIndex = NDSContext_addStringConstant(ctx, nameData, nameLength);
    if (nameIndex == NDS_NO_CONSTANT)
        Compiler_error(self, "out of memory");
...
Value_equals(NDSContext *ctx, NDSValue a, NDSValue b, bool *out)
{
    ...
    VM_DISPATCH_SETUP(ctx, 2, return NDSStatus_Error);
    ...
    if (status != NDSStatus_OK)
        return status;
    *out = result.as.boolean;
    return NDSStatus_OK;
}

static NDSStatus
Value_compare(NDSContext *ctx, NDSValue a, NDSValue b, NDSSlotComparison *out)
{
    if (a.type != b.type){
        *out = NDSSlotComparison_Incomparable;
        return NDSStatus_OK;
    }
    TypeDescriptor *desc = NDSContext_getTypeDesc(ctx, a.type);
    VM_DISPATCH_SETUP(ctx, 2, return NDSStatus_Error);
    ...
    if (cmp < 0)
        *out = NDSSlotComparison_LessThan;
    else if (cmp > 0)
        *out = NDSSlotComparison_GreaterThan;
    else
        *out = NDSSlotComparison_Equal;
    return NDSStatus_OK;
}
...
{
    NDSSlotComparison cmp;
    if (Value_compare(self, NDSContext_getSlot(self, slotA), NDSContext_getSlot(self, slotB), &cmp) != NDSStatus_OK)
...
{
    bool eq = false;
    if (Value_equals(self, NDSContext_getSlot(self, slotA), NDSContext_getSlot(self, slotB), &eq) != NDSStatus_OK)
        return false;
    return eq;
}
...
/* Lists and maps clone; everything else is copied by reference. */
static NDSStatus
Util_shallowCopySlot(NDSContext *ctx, int srcSlot, int destSlot)
{
    NDSValue val = NDSContext_getSlot(ctx, srcSlot);
    if (val.type == NDSValueType_List && val.as.object){
        if (NDSContext_setSlotNewList(ctx, destSlot) != NDSStatus_OK)
            return NDSStatus_Error;
        NDSObject *newObj = NDSContext_slotPointer(ctx, destSlot)->as.object;
        List *src = (List *)NDSObject_getExtraData(ctx, val.as.object);
        size_t count = src->count;
        for (size_t i = 0; i < count; i++){
            src = (List *)NDSObject_getExtraData(ctx, NDSContext_getSlot(ctx, srcSlot).as.object);
    ...
    if (val.type == NDSValueType_Map && val.as.object){
        if (NDSContext_setSlotNewMap(ctx, destSlot) != NDSStatus_OK)
            return NDSStatus_Error;
        NDSObject *newObj = NDSContext_slotPointer(ctx, destSlot)->as.object;
        Map *src = (Map *)NDSObject_getExtraData(ctx, val.as.object);
        size_t entryCount = src->entries? Map_entries(ctx, src)->count : 0;
        for (size_t i = 0; i < entryCount; i++){
            src = (Map *)NDSObject_getExtraData(ctx, NDSContext_getSlot(ctx, srcSlot).as.object);
            NDSValue entryVal = Map_entries(ctx, src)->items[i];
            if (entryVal.type == NDSValueType_Nothing)
                continue;
            NDSValue key = NDSObject_getProperty(entryVal.as.object, MapEntryProp_Key);
            NDSValue value = NDSObject_getProperty(entryVal.as.object, MapEntryProp_Value);
            if (Map_set(ctx, newObj, key, value) < 0){
                NDSContext_setError(ctx, "out of memory");
                return NDSStatus_Error;
            }
        }
    ...
    NDSContext_copySlot(ctx, srcSlot, destSlot);
    return NDSStatus_OK;
}

static NDSStatus
Builtin_copy(NDSContext *ctx, int argCount)
{
    UNUSED(argCount);
    if (Util_shallowCopySlot(ctx, 0, NDSContext_VarSlot(0)) != NDSStatus_OK)
        return NDSStatus_Error;
...
static NDSStatus
Builtin_incrementOrDecrement(NDSContext *ctx, double delta)
{
    NDSValue *slot = NDSContext_slotPointer(ctx, NDSContext_VarSlot(0));
    if (slot->type != NDSValueType_Number){
        NDSContext_setError(ctx, "increment/decrement requires a number");
        return NDSStatus_Error;
    }
    slot->as.number += delta;
    NDSContext_setSlotNothing(ctx, 0);
    return NDSStatus_OK;
}

static NDSStatus
Builtin_increment(NDSContext *ctx, int argCount)
{
    UNUSED(argCount);
    return Builtin_incrementOrDecrement(ctx, 1.0);
}

static NDSStatus
Builtin_decrement(NDSContext *ctx, int argCount)
{
    UNUSED(argCount);
    return Builtin_incrementOrDecrement(ctx, -1.0);
}
    ...
    do{                                                                                            \
        ...
        for (;;){                                                                                  \
            ...
            if (child_ >= (end_))                                                                  \
                break;                                                                             \
            if (child_ + 1 < (end_)){                                                              \
                NDSSlotComparison cmp_;                                                            \
                if (Value_compare(ctx, items[child_], items[child_ + 1], &cmp_) != NDSStatus_OK)   \
                    return NDSStatus_Error;                                                        \
                if (cmp_ == NDSSlotComparison_LessThan)                                            \
                    child_++;                                                                      \
            }                                                                                      \
            NDSSlotComparison cmp2_;                                                               \
            if (Value_compare(ctx, items[parent_], items[child_], &cmp2_) != NDSStatus_OK)         \
                return NDSStatus_Error;                                                            \
            if (cmp2_ != NDSSlotComparison_LessThan)                                               \
                break;                                                                             \
...
Builtin_sort(NDSContext *ctx, int argCount)
{
    ...
    if (list->count > 1){
        ...
        if (Util_heapSort(ctx, list->items, list->count) != NDSStatus_OK)
            return NDSStatus_Error;
    }
...
NDSContext_setupStdlib(NDSContext *self)
{
    ...
    NDSPatternStep copyPattern[] = {
        { NDSPatternStep_Word, "copy"},
        { NDSPatternStep_Expression, NULL},
        { NDSPatternStep_Word, "to"},
        { NDSPatternStep_Variable, NULL},
        { NDSPatternStep_End, NULL}
    };
    if (NDSConfigHandle_registerCommand(&handle, copyPattern, Builtin_copy) != NDSStatus_OK)
        return NDSStatus_Error;

    NDSPatternStep incrementPattern[] = {
        { NDSPatternStep_Word, "increment"},
        { NDSPatternStep_Reference, NULL},
        { NDSPatternStep_End, NULL}
    };
    if (NDSConfigHandle_registerCommand(&handle, incrementPattern, Builtin_increment) != NDSStatus_OK)
        return NDSStatus_Error;

    NDSPatternStep decrementPattern[] = {
        { NDSPatternStep_Word, "decrement"},
        { NDSPatternStep_Reference, NULL},
        { NDSPatternStep_End, NULL}
    };
    if (NDSConfigHandle_registerCommand(&handle, decrementPattern, Builtin_decrement) != NDSStatus_OK)
        return NDSStatus_Error;
...
#define VM_COMPARE(cmp1, cmp2)                                                                     \
    do{                                                                                           \
        ...
        NDSValue a = VM_pop(ctx);                                                                  \
        NDSSlotComparison cmp;                                                                     \
        if (Value_compare(ctx, a, b, &cmp) != NDSStatus_OK){                                       \
            VM_push(ctx, NDSValue_Nothing);                                                        \
            goto vm_error;                                                                         \
        }                                                                                          \
        if (cmp == NDSSlotComparison_Incomparable){                                               \
...
void NDS_PRINTF(2, 3)
NDSContext_setErrorF(NDSContext *ctx, const char *fmt, ...)
{
    if (ctx->errorMessage[0] != '\0')
        return;
    va_list ap;
...
VM_concat(NDSContext *ctx)
{
    ...
    /* This stays rooted in the return slot. */
    char *buf = NDSContext_newStringUninit(ctx, newLen, &ctx->stack[ctx->stackTop - 2]);
    if (!buf)
        return false;
    memcpy(buf, sdA->data, sdA->length);
    memcpy(buf + sdA->length, sdB->data, sdB->length);
    ctx->stackTop--;
    return true;
}
...
/* Call a script function (either a function or a given block). */
static NDSStatus
VM_invokeScript(NDSContext *ctx, Function *func, size_t callLocalBase,
                uint32_t attachedBlock, size_t closureBase, size_t callerLocalBase)
{
    VMCallInfo callInfo = {
        .closureBase = closureBase,
        .block = attachedBlock,
        .callerLocalBase = callerLocalBase,
           ...
           size_t localBase, const VMCallInfo *info)
{
    ...
    while (ip < codeLength){
        ...
        case Op_Eq: {
            ...
            NDSValue a = VM_pop(ctx);
            bool eq = false;
            if (Value_equals(ctx, a, b, &eq) != NDSStatus_OK){
                VM_push(ctx, NDSValue_Nothing);
                goto vm_error;
            }
            VM_push(ctx, NDSValue_booleanFromBool(eq));
            break;
        }
        case Op_Neq: {
            NDSValue b = VM_pop(ctx);
        ...
        case Op_Call: {
            ...
            if (func->kind == FuncKind_Host){
                if (VM_callHost(ctx, func, callLocalBase, argCount) != NDSStatus_OK)
                    goto vm_error;
            } else if (VM_invokeScript(ctx, func, callLocalBase, inst->b, 0, localBase) != NDSStatus_OK){
                goto vm_error;
            }
            break;
        }
        ...
        case Op_PropertyGet: {
            ...
            VM_DISPATCH_PUSH(ctx, container);
            VM_DISPATCH_FRAME(ctx);
            NDSStatus status_ = Type_dispatchPropertyGet(ctx, propAtom);
            NDSValue result;
            VM_DISPATCH_TEARDOWN(ctx, result);
            if (status_ != NDSStatus_OK){
        ...
        case Op_FillSlot: {
            uint32_t depth = inst->a;
            if (ctx->stackTop == 0 || (size_t)depth + 1 > ctx->stackTop){
                NDSContext_setError(ctx, "stack underflow");
                goto vm_error;
            }
            NDSValue val = VM_pop(ctx);
            ctx->stack[ctx->stackTop - 1 - depth] = val;
            break;
        }
        case Op_ChunkSet: {
        ...
        case Op_CallGiven: {
            ...
            if (VM_invokeScript(ctx, func, callLocalBase, NDS_NO_BLOCK, callerLocalBase, 0) != NDSStatus_OK)
                goto vm_error;
            break;
        }
        case Op_BlockGiven:
...
NDSContext_getGlobal(NDSContext *self, const char *name, int destSlot)
{
    ...
    if (atom == NDSAtom_Invalid)
        return false;
    uint32_t idx = Globals_find(self, atom);
    if (idx == NDS_NO_GLOBAL)
        return false;
...
NDSContext_setGlobal(NDSContext *self, const char *name, int srcSlot)
{
    ...
    uint32_t idx = Globals_ensure(self, atom);
    if (idx == NDS_NO_GLOBAL){
        NDSContext_setError(self, "out of memory");
        return NDSStatus_Error;
    }
...
NDSContext_callFuncIdx(NDSContext *self, uint32_t funcIdx, int argCount)
{
    ...
    if (func->kind == FuncKind_Host)
        ...
        status = VM_invokeScript(self, func, callBase, NDS_NO_BLOCK, 0, 0);
...
NDSContext_callFunction(NDSContext *self, const char *name, int argCount)
{
    ...
    uint32_t globalIdx = Globals_find(self, atom);
    if (globalIdx == NDS_NO_GLOBAL || self->globals[globalIdx].value.type != NDSValueType_Function){
        NDSContext_setError(self, "unknown function");
        return NDSStatus_Error;
    }
