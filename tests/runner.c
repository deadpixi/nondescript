...
/* frob X (X is Reference): doubles X. */
static NDSStatus
Command_frob(NDSContext *ctx, int argCount)
{
    UNUSED(argCount);
    int xSlot = NDSContext_VarSlot(0);
    if (!NDSContext_isSlotNumber(ctx, xSlot)){
        NDSContext_setError(ctx, "frob requires a number");
        return NDSStatus_Error;
    }
    NDSContext_setSlotNumber(ctx, xSlot, NDSContext_getSlotNumber(ctx, xSlot) * 2.0);
    return NDSStatus_OK;
}

/* clamp X between LO and HI (X is Reference): clamps X into [LO, HI]. */
static NDSStatus
Command_clamp(NDSContext *ctx, int argCount)
{
    UNUSED(argCount);
    int xSlot = NDSContext_VarSlot(0);
    if (!NDSContext_isSlotNumber(ctx, xSlot) ||
        !NDSContext_isSlotNumber(ctx, 0) ||
        !NDSContext_isSlotNumber(ctx, 1)){
        NDSContext_setError(ctx, "clamp requires a number");
        return NDSStatus_Error;
    }
    double v = NDSContext_getSlotNumber(ctx, xSlot);
    double lo = NDSContext_getSlotNumber(ctx, 0);
    double hi = NDSContext_getSlotNumber(ctx, 1);
    if (v < lo) v = lo;
    if (v > hi) v = hi;
    NDSContext_setSlotNumber(ctx, xSlot, v);
    return NDSStatus_OK;
}

/* swap X with Y (both Reference): swaps the values of X and Y. */
static NDSStatus
Command_swapRef(NDSContext *ctx, int argCount)
{
    UNUSED(argCount);
    int xSlot = NDSContext_VarSlot(0);
    int ySlot = NDSContext_VarSlot(1);
    NDSContext_ensureSlots(ctx, 1);  /* scratch slot for the swap */
    int tmpSlot = NDSContext_getSlotCount(ctx); /* 0-based; ensureSlots guarantees this */
    UNUSED(tmpSlot);
    /* Use slot 0 (the return slot) as scratch since no host args here. */
    NDSContext_copySlot(ctx, xSlot, 0);
    NDSContext_copySlot(ctx, ySlot, xSlot);
    NDSContext_copySlot(ctx, 0, ySlot);
    return NDSStatus_OK;
}
...
setup(NDSConfigHandle *h, void *userPointer)
{
    ...
    NDSPatternStep frobCmd[] ={
        {NDSPatternStep_Word, "frob"},
        {NDSPatternStep_Reference, NULL},
        {NDSPatternStep_End, NULL}
    };
    NDSConfigHandle_registerCommand(h, frobCmd, Command_frob);

    NDSPatternStep clampCmd[] ={
        {NDSPatternStep_Word, "clamp"},
        {NDSPatternStep_Reference, NULL},
        {NDSPatternStep_Word, "between"},
        {NDSPatternStep_Expression, NULL},
        {NDSPatternStep_Word, "and"},
        {NDSPatternStep_Expression, NULL},
        {NDSPatternStep_End, NULL}
    };
    NDSConfigHandle_registerCommand(h, clampCmd, Command_clamp);

    NDSPatternStep swapRefCmd[] ={
        {NDSPatternStep_Word, "swapref"},
        {NDSPatternStep_Reference, NULL},
        {NDSPatternStep_Word, "and"},
        {NDSPatternStep_Reference, NULL},
        {NDSPatternStep_End, NULL}
    };
    NDSConfigHandle_registerCommand(h, swapRefCmd, Command_swapRef);
